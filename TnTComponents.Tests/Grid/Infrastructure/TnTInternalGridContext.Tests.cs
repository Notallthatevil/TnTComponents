...
public class TnTInternalGridContext_Tests : BunitContext {
    ...
    public void Constructor_WithNullGrid_AcceptsNullParameter() {
        // Arrange & Act
        // Note: The primary constructor parameter doesn't enforce null checking
        // This tests the actual behavior of the constructor
    ...
    #region Column Registration Tests
    ...
    public void UpdateItems_WithPagination_AppliesPagination() {
        ...
        grid.Items = _testItems.AsQueryable();
        var pagination = new TnTPaginationState();
        // Assume pagination has methods to set page size and current page
        // This test verifies the integration with pagination
        grid.Pagination = pagination;
        var context = new TnTInternalGridContext<TestGridItem>(grid);
        ...
        // Assert
        // The actual result depends on pagination implementation
        // This verifies that pagination is considered in the pipeline
    ...
    public void EdgeCase_RemoveSortedColumn_UpdatesSortingState() {
        ...
        // Assert
        context.Columns.Should().NotContain(column);
        // The sorting state should remain (it's not automatically cleared when column is removed)
        // This tests the current behavior - may need adjustment based on requirements
    }
