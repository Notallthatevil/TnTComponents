...
// One awkwardness of the way TnTDataGrid collects its list of child columns is that, during
...
// have the latest set of columns
// - Drawback: since TnTDataGrid has public API, it's much messier to have to forward all of that to
// some new child type.
// - However, this is arguably the most correct solution in general (at least until option 1 above
// is implemented)
// - Or, we could decide it's enough to fix this on the first render (since that's the only time
...
public sealed class ColumnsCollectedNotifier<TGridItem> : Microsoft.AspNetCore.Components.IComponent {
    [CascadingParameter] internal TnTInternalGridContext<TGridItem> InternalGridContext { get; set; } = default!;
