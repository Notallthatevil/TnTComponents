...
internal interface IAsyncQueryExecutor {
    ...
    /// <summary>
    ///     Determines whether the <see cref="IQueryable{T}" /> is supported by this <see
    ///     cref="IAsyncQueryExecutor" /> type.
    /// </summary>
    /// <typeparam name="T">The data type.</typeparam>
    /// <param name="queryable">An <see cref="IQueryable{T}" /> instance.</param>
    /// <returns>
    ///     True if this <see cref="IAsyncQueryExecutor" /> instance can perform asynchronous
    ///     queries for the supplied <paramref name="queryable" />, otherwise false.
    /// </returns>
...
internal static class AsyncQueryExecutorSupplier {
    ...
    // extensions and don't just fall back on the synchronous IQueryable ToXyz calls
    ...
    public static IAsyncQueryExecutor? GetAsyncQueryExecutor<T>(IServiceProvider services, IQueryable<T>? queryable) {
        if (queryable is not null) {
            ...
            if (executor is null) {
                // It's useful to detect if the developer is unaware that they should be using the
                // EF adapter, otherwise they will likely never notice and simply deploy an
                // inefficient app that blocks threads on each query.
    ...
    // We have to do this via reflection because the whole point is to avoid any static dependency
    // on EF unless you reference the adapter. Trimming won't cause us any problems because this is
    // only a way of detecting misconfiguration so it's sufficient if it can detect the
