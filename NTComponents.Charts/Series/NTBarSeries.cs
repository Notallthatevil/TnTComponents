...
public class NTBarSeries<TData> : NTCartesianSeries<TData> where TData : class
{
   ...
   public override void Render(SKCanvas canvas, SKRect renderArea)
   {
      ...
      float myVisibilityFactor = VisibilityFactor;
   ...
   private List<SKRect> GetBarRects(SKRect renderArea, double xMin, double xMax, double yMin, double yMax, double yBase)
   {
      ...
      float barSeriesWeightTotal = Math.Max(0.001f, Chart.GetBarSeriesTotalWeight());
      float barSeriesOffsetWeight = Chart.GetBarSeriesOffsetWeight(this);
      float myVisibilityFactor = VisibilityFactor;
      ...
      float barWidth = (groupWidth / barSeriesWeightTotal) * myVisibilityFactor;
      ...
      for (int i = 0; i < dataList.Count; i++)
      {
         ...
         float groupStart = centerX - groupWidth / 2;
         float barX = groupStart + (barSeriesOffsetWeight * (groupWidth / barSeriesWeightTotal)) + (barWidth / 2);
